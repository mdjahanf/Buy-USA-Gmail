...
async function startServer() {
  ...
  app.get('/sitemap.xml', (req, res) => {
    ...
    const servicePages = servicesStore.map(s => ({
      url: `/services/${s.slug}`,
      priority: '0.95',
      changefreq: 'weekly'
    }));

    const blogPages = blogsStore.map(b => ({
      url: `/blog/${b.slug}`,
      priority: '0.8',
      changefreq: 'weekly'
    }));
...
`;
  ...
  // ==========================================
  // 6. DYNAMIC SERVER-SIDE SEO & STATIC ASSETS
  // ==========================================
  function generateHTMLWithSEO(reqPath: string, template: string): string {
    const cleanPath = reqPath.split('?')[0].replace(/\/+$/, '') || '/';
    
    let title = 'Buy USA Gmail Accounts - Aged, PVA & Bulk Accounts | buyusagmail.com';
    let description = 'Buy USA Aged & PVA Gmail accounts with instant delivery, 72-hr warranty, non-VoIP SIM verification, and 2-8 yrs history. Order bulk accounts safely.';
    let canonicalUrl = `https://buyusagmail.com${cleanPath === '/' ? '' : cleanPath}`;
    let jsonLd: any = null;
    ...
    if (cleanPath.startsWith('/services/')) {
      ...
      if (service) {
        title = `${service.name} - Buy Aged USA Gmail Accounts | buyusagmail.com`;
        description = `${service.overview.slice(0, 155)}... Guaranteed 72-hour replacement warranty.`;
        jsonLd = {
      ...
      const post = blogsStore.find(p => p.slug === slug);
      if (post) {
        title = `${post.title} | buyusagmail.com Research`;
      ...
      description = 'Explore 50+ deep-dive research studies on cold email deliverability, anti-detect browsers, residential proxies, and GMB review stickiness.';
    } else if (cleanPath === '/pricing') {
      title = 'Wholesale Pricing & Bulk Discount Calculator | buyusagmail.com';
      description = 'Calculate wholesale discounts up to 60% off for USA Aged Gmail, PVA accounts, and aged Google Ads profiles.';
    } else if (cleanPath === '/about') {
      title = 'About Us - Enterprise Email Infrastructure | buyusagmail.com';
      description = 'Learn about buyusagmail.com, the premier wholesale marketplace for aged USA residential Gmail accounts and non-VoIP PVA profiles.';
    } else if (cleanPath === '/faq' || cleanPath === '/faqs') {
      title = 'Frequently Asked Questions (FAQ) | buyusagmail.com';
      description = 'Find instant answers regarding proxy setup, 72-hour replacement warranty, crypto payment verification, and bulk email limits.';
    } else if (cleanPath === '/contact') {
      title = '24/7 Support Desk & Direct Contact | buyusagmail.com';
      description = 'Get instant 24/7 technical support via Telegram (@EgSupport24) or WhatsApp for your bulk Gmail orders.';
    } else if (cleanPath === '/sitemap') {
      title = 'HTML Sitemap & Complete Directory | buyusagmail.com';
      description = 'Complete index directory of all services, 50 SEO guides, legal policies, and tools on buyusagmail.com.';
    } else if (cleanPath === '/terms') {
      title = 'Terms of Service & Wholesale Agreement | buyusagmail.com';
      description = 'Review the official wholesale terms of service and usage policies for buyusagmail.com aged USA and PVA accounts.';
    } else if (cleanPath === '/privacy') {
      title = 'Privacy Policy & Data Protection | buyusagmail.com';
      description = 'Learn how buyusagmail.com encrypts order data and maintains zero-log anonymity for cryptocurrency purchasers.';
    } else if (cleanPath === '/refund') {
      title = '72-Hour Replacement Warranty & Refund Policy | buyusagmail.com';
      description = 'Examine our industry-leading 72-hour replacement guarantee against initial login checkpoints and password mismatches.';
    }
    ...
    if (modified.includes('<title>')) {
      modified = modified.replace(/<title>.*?<\/title>/s, headInject);
    } else {
      modified = modified.replace('</head>', `${headInject}\n</head>`);
    }
    return modified;
  }
    ...
    app.get('*', async (req, res, next) => {
      ...
      try {
        ...
        if (fs.existsSync(indexPath)) {
          ...
          const finalHtml = generateHTMLWithSEO(req.path, template);
    ...
    app.get('*', (req, res) => {
      try {
        ...
        if (fs.existsSync(templatePath)) {
          ...
          const finalHtml = generateHTMLWithSEO(req.path, rawHtml);
          res.setHeader('Content-Type', 'text/html; charset=utf-8');
          return res.send(finalHtml);
        }
      } catch (err) {
        console.error('Error serving dynamic HTML SEO:', err);
      }
