...
export const SitemapPage: React.FC<{ onNavigate: (r: any, s?: any) => void; onBack: () => void }> = ({ onNavigate, onBack }) => (
  <div className="space-y-8 py-12 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto font-sans text-[#202124] animate-in fade-in duration-300 text-xs sm:text-sm leading-relaxed">
    ...
    <a href="/" onClick={(e) => { e.preventDefault(); onBack(); }} className="flex items-center gap-2 text-xs font-bold text-[#4285F4] hover:underline"><ArrowLeft className="w-4 h-4" /> Back to Home</a>
    <div className="p-8 rounded-[32px] bg-white dark:bg-gray-900 border border-[#E5E7EB] dark:border-gray-800 space-y-6 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 font-bold">
        <div className="space-y-2">
          ...
          <ul className="space-y-1.5 font-medium text-[#4285F4]">
            <li><a href="/" onClick={(e) => { e.preventDefault(); onNavigate('home'); }}>&rarr; Home</a></li>
            <li><a href="/services" onClick={(e) => { e.preventDefault(); onNavigate('services'); }}>&rarr; All Services</a></li>
            <li><a href="/pricing" onClick={(e) => { e.preventDefault(); onNavigate('pricing'); }}>&rarr; Wholesale Pricing</a></li>
            <li><a href="/about" onClick={(e) => { e.preventDefault(); onNavigate('about'); }}>&rarr; About buyusagmail.com</a></li>
            <li><a href="/faq" onClick={(e) => { e.preventDefault(); onNavigate('faq'); }}>&rarr; FAQ Hub</a></li>
            <li><a href="/blog" onClick={(e) => { e.preventDefault(); onNavigate('blog'); }}>&rarr; SEO Knowledge Hub</a></li>
            <li><a href="/payment-methods" onClick={(e) => { e.preventDefault(); onNavigate('payment-methods'); }}>&rarr; Crypto Payment Guide</a></li>
            <li><a href="/contact" onClick={(e) => { e.preventDefault(); onNavigate('contact'); }}>&rarr; Direct Contact Desk</a></li>
          </ul>
        </div>
        <div className="space-y-2">
          ...
          <ul className="space-y-1.5 font-medium text-[#4285F4]">
            {(SERVICES_DATA || []).map((s) => (
              <li key={s.id}>
                <a href={`/services/${s.slug}`} onClick={(e) => { e.preventDefault(); onNavigate('service-detail', s.slug); }} className="text-left hover:underline">
                  &rarr; {s.name}
                </a>
              </li>
            ))}
        ...
        <div className="space-y-2 col-span-1 sm:col-span-2 lg:col-span-2">
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1.5 font-medium text-[#4285F4]">
            {(BLOG_POSTS || []).map((b) => (
              <li key={b.id}>
                <a href={`/blog/${b.slug}`} onClick={(e) => { e.preventDefault(); onNavigate('blog-post', b.slug); }} className="text-left hover:underline line-clamp-1">
                  &rarr; {b.title}
                </a>
              </li>
            ))}
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-1.5 font-medium text-[#4285F4]">
            <li><a href="/terms" onClick={(e) => { e.preventDefault(); onNavigate('terms'); }}>&rarr; Terms of Service</a></li>
            <li><a href="/privacy" onClick={(e) => { e.preventDefault(); onNavigate('privacy'); }}>&rarr; Privacy Policy</a></li>
            <li><a href="/refund" onClick={(e) => { e.preventDefault(); onNavigate('refund'); }}>&rarr; 72-Hour Refund Policy</a></li>
            <li><a href="/dmca" onClick={(e) => { e.preventDefault(); onNavigate('dmca'); }}>&rarr; DMCA Copyright</a></li>
            <li><a href="/disclaimer" onClick={(e) => { e.preventDefault(); onNavigate('disclaimer'); }}>&rarr; Legal Disclaimer</a></li>
          </ul>
        </div>
      </div>
    </div>
